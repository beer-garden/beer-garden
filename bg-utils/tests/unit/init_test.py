...
class BgutilsTest(unittest.TestCase):
    ...
    def setUp(self):
        self.spec = ConfigSpec({'log_config': {'required': False, 'default': None},
    ...
    def test_parse_args(self):
        cli_args = ["--log_config", "/path/to/log/config", "--log_file", "/path/to/log/file", "--log_level", "INFO",
    ...
    def test_generate_config_no_file(self):
        self.spec.output_config = Mock()
        bg_utils.generate_config(self.spec, [])
        self.spec.output_config.assert_called_with({"log_file": None, "log_level": "INFO",
                                                    "log_config": None, "config": None})

    def test_generate_config_with_file(self):
        self.spec.output_config = Mock()
        bg_utils.generate_config(self.spec, ["--config", "/path/to/config"])
        self.spec.output_config.assert_called_with({"log_file": None, "log_level": "INFO",
                                                    "log_config": None, "config": "/path/to/config"},
                                                   output_file_path="/path/to/config",
