...
class SystemListAPITest(TestUtils, unittest.TestCase):
    ...
    def test_get(self, order_mock):
        ...
        response_systems = self.parser.parse_system(response.data, many=True, from_string=True)
    ...
    def test_get_no_include_commands(self, order_mock):
        ...
        response_systems = self.parser.parse_system(response.data, many=True, from_string=True)
        self.assertEqual(1, len(response_systems))
        self._assert_systems_equal(self.default_system, response_systems[0], include_commands=False)
    ...
    def test_post_check_calls(self, url_for_mock, parse_mock):
        ...
        with patch('brew_view.controllers.system_list_api.jsonify', jsonify_mock):
            rv = self.app.post('/api/v1/systems', data='{}',
    ...
    def test_post_check_headers(self, parse_mock):
        ...
        self.assertEqual(rv.headers['Location'],
                         'http://localhost/api/v1/systems/id')
        self.assertEqual(200, rv.status_code)
