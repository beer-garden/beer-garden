...
class TestComplex(object):
    ...
    def test_echo_model(self):
        parameters = {
            "model": {
                ...
                "my_list_of_models": [
                    {
                        "my_list_of_strings": ["more", "list", "of", "strings"],
                        "my_choices_string": "a",
                    },
                    {
                        "my_list_of_strings": ["more", "list", "of", "strings2"],
                        "my_choices_string": "b",
                    },
                ],
            }
    ...
    def test_echo_optional_multi_nullable_model_with_multi_defaults(self):
        ...
        response = wait_for_response(self.easy_client, request)
        assert_successful_request(response)
        assert [{"my_nested_string": "hi", "my_nested_int": 2}] == json.loads(
            response.output
        )
    ...
    def test_echo_simple_model(self):
        ...
        response = wait_for_response(self.easy_client, request)
        assert_successful_request(response)
        assert {"my_nested_string": "str1", "my_nested_int": 1} == json.loads(
            response.output
        )
    ...
    def test_echo_simple_model_with_default(self):
        ...
        response = wait_for_response(self.easy_client, request)
        assert_successful_request(response)
        assert {"my_nested_string": "foo", "my_nested_int": 1} == json.loads(
            response.output
        )
    ...
    def test_prove_env(self):
        ...
        response = wait_for_response(self.easy_client, request)
        assert_successful_request(response)
        assert {"DB_NAME": "complex", "DB_PASS": "supersecret"} == json.loads(
            response.output
        )
