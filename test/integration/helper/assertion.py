...
def assert_system_running(client, system_name, system_version, garden=None, **kwargs):
    system = client.find_unique_system(
        name=system_name, version=system_version, garden=garden
    )
...
def assert_error_creating_request(
    testcase, client, request, exc_class, regex=None, **kwargs
):
    if regex is None:
        with pytest.raises(exc_class) as ex:
            created_request = client.create_request(request)
            print(
                "Uh-oh. Request (%s) should have errored, but didn't."
                % created_request.id
            )
    else:
        with pytest.raises(exc_class) as ex:
    ...
    for k in kwargs.keys():
        assert (
            getattr(the_exception, k) == kwargs[k]
        ), "Exception was thrown, but %s did " "not match. Expected (%s) got (%s)" % (
            k,
            kwargs[k],
            getattr(the_exception, k),
        )
...
def assert_request(request, **kwargs):
    for (key, expected) in kwargs.items():
        ...
        if key == "output":
            ...
            assert actual == expected, "%s did not match. Expected (%s) got (%s)" % (
                key,
                expected,
                actual,
            )
