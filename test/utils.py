...
def convert(model, serializer=None, parser=None):
    ...
    return parser._do_parse(
        serializer._do_serialize(schema(), model, True),
        schema(),
        True,
    )


mongo2brew = partial(convert, serializer=BeerGardenSchemaParser, parser=SchemaParser)
brew2mongo = partial(convert, serializer=SchemaParser, parser=BeerGardenSchemaParser)
