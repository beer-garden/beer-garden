...
describe("<UsersAddContainer />", () => {
  ...
  describe("handleSubmit", () => {
    it("should save the user and set redirect to true if everything works", async () => {
      const {
        container,
        props: { createUser },
      } = setup(
      ...
      expect(createUser).toHaveBeenCalledWith("name", "asdfQWER1234!", [
        "role1",
      ]);
      expect(container.state().redirect).toBe(true);
    });

    it("should not redirect if saving the user fails", async () => {
