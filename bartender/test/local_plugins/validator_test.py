...
class LocalPluginValidatorTest(unittest.TestCase):
    ...
    @patch('bartender.local_plugins.validator.LocalPluginValidator.validate_instances_and_args', Mock())
    @patch('bartender.local_plugins.validator.LocalPluginValidator.validate_plugin_environment', Mock())
    ...
    def test_validate_plugin_load_plugin_config(self, mock_load):
        ...
        mock_load.assert_called_with('BGPLUGINCONFIG', '/path/to/plugin/%s' % self.validator.CONFIG_NAME)
    ...
    def test_validate_plugin_path_bad(self):
        self.assertRaises(PluginValidationError, self.validator.validate_plugin_path, '/path/to/non-existant/foo')
    ...
    def test_validate_plugin_config_not_a_file(self, isfile_mock):
        ...
        self.assertRaises(PluginValidationError, self.validator.validate_plugin_config, 'not_a_file')
    ...
    def test_validate_entry_point_none_config_module(self):
        self.assertRaises(PluginValidationError, self.validator.validate_entry_point, None, '/path/to/plugin')
    ...
    def test_validate_entry_point_no_entry_point_key(self):
        self.assertRaises(PluginValidationError, self.validator.validate_entry_point, Mock(spec=[]), '/path/to/plugin')
    ...
    def test_validate_entry_point_bad_entry_point(self):
        self.assertRaises(PluginValidationError, self.validator.validate_entry_point, Mock(
            spec=[self.validator.ENTRY_POINT_KEY], PLUGIN_ENTRY='not_a_file'), '/path/to/plugin')

    @patch('bartender.local_plugins.validator.isfile', Mock(return_value=True))
    def test_validate_entry_point_good_file(self):
        self.validator.validate_entry_point(Mock(PLUGIN_ENTRY='is_totally_a_file'), '/path/to/plugin')
    ...
    def test_validate_individual_plugin_arguments_not_none_or_list(self):
        self.assertRaises(PluginValidationError, self.validator.validate_individual_plugin_arguments, 'notalistornone')
    ...
    def test_validate_individual_plugin_arguments_bad_list(self):
        self.assertRaises(PluginValidationError, self.validator.validate_individual_plugin_arguments, [{'foo': 'bar'}])
    ...
    def test_validate_plugin_environment_good(self):
        self.assertTrue(self.validator.validate_plugin_environment(Mock(ENVIRONMENT={'foo': 'bar'})))
    ...
    def test_validate_instances_and_args_both_none(self):
        ...
        self.assertTrue(PluginValidationError, self.validator.validate_instances_and_args(config_module))

    def test_validate_instances_and_args_invalid_instances(self):
        config_module = Mock(INSTANCES="THIS_IS_WRONG", PLUGIN_ARGS=None)
        self.assertRaises(PluginValidationError, self.validator.validate_instances_and_args, config_module)
