...
class EchoSleeperClient:
    ...
    def __init__(self):
        ...
        self.multi_sleeper_client = SystemClient(host, port, 'multi-sleeper', ssl_enabled=ssl_enabled, blocking=False)
        self.error_client = SystemClient(host, port, 'error', ssl_enabled=ssl_enabled)

    @parameter(key="message", description="The message", optional=True, type="String", default=DEFAULT_MESSAGE)
    @parameter(key="loud", description="Add exclamation marks", optional=True, type="Boolean", default=False)
    @parameter(key="amount", description="How long to sleep", optional=True, type="Float", default=10)
    ...
    @parameter(key="loud", description="Add exclamation marks", optional=True, type="Boolean", default=False)
    @parameter(key="amount", description="How long to sleep", optional=True, type="Float", default=10)
    @parameter(key="number", description="Number of times to sleep", optional=True, type="Integer", default=1)
    ...
    def super_sleeper_concurrent(self, message=DEFAULT_MESSAGE, loud=False, amount=10, number=1):
        ...
        sleeps = [self.multi_sleeper_client.sleep(amount=amount, _comment=number) for number in range(number)]
