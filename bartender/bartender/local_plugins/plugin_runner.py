...
class LocalPluginRunner(StoppableThread):
    """Class for running a local plugin in its own process. Can be stopped/started and killed like a normal process"""

    def __init__(self, entry_point, system, instance_name, path_to_plugin, web_host, web_port, ssl_enabled,
                 plugin_args=None, environment=None, requirements=None, plugin_log_directory=None, url_prefix=None,
                 ca_verify=True, ca_cert=None):
        ...
        self.logger = getPluginLogger(self.unique_name, formatted=False, log_directory=self.plugin_log_directory)
    ...
    def status(self):
        try:
            # TODO: Remove this reload. We have to find a better way to store the status that doesn't
            # require a database connection. (issue/217)
    ...
    def status(self, value):
        try:
            ...
            self.logger.error("Error updating status of plugin %s to %s" % (self.unique_name, value))
    ...
    def run(self):
        ...
        Run the plugin using the entry point specified with the generated environment in its own subprocess. Pipes
        STDOUT and STDERR such that when the plugin stops executing (or IO is flushed) it will log it.
        """
        try:
            self.logger.info("Starting plugin %s subprocess: %s", self.unique_name, self.executable)
            self.process = subprocess.Popen(self.executable, bufsize=0, env=self._generate_plugin_environment(),
                                            stdout=subprocess.PIPE, stderr=subprocess.STDOUT,
                                            cwd=os.path.abspath(self.path_to_plugin),
                                            preexec_fn=lambda: signal.signal(signal.SIGINT, signal.SIG_IGN))

            # Reading the process IO is blocking and we want to be able to shutdown gracefully when requested
            # So reading the IO needs to be in its own thread
            ...
            self.logger.info("Plugin %s subprocess has stopped with exit status %s, performing final IO read(s)",
                             self.unique_name, self.process.poll())
            io_thread.join()
    ...
    def _check_io(self):
        ...
        This method will read from STDERR. If the lines include one of the logging Levels that the python logger
        knows about we assume that the plugin has its own logger and its own formatter. As such, we will log to our
        unformatted logger so that we preserve the original formatting.

        If the plugin is just using STDOUT with no logging then generally the logging level will not be included and so
        we just add the normal log formatting.
        """
