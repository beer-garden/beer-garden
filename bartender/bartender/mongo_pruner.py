...
class MongoPruner(StoppableThread):
    ...
    def add_task(self, collection=None, field=None, delete_after=None, additional_query=None):
        self._tasks.append({'collection': collection, 'field': field, 'delete_after': delete_after,
    ...
    def run(self):
        ...
        while not self.wait(self._run_every):
            ...
            for task in self._tasks:
                ...
                self.logger.debug("Removing %ss older than %s" % (task['collection'].__name__, str(delete_older_than)))
