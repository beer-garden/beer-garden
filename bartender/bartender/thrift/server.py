...
class BartenderThriftServer(TThreadedServer, StoppableThread):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.pool = ThreadPoolExecutor(max_workers=bartender.config.max_thrift_workers)
        self.futures = set()
        self.finished = Event()

        StoppableThread.__init__(self, logger=self.logger, name=kwargs.pop('name', 'ThriftPyServer'))
    ...
    def run(self):
        ...
        # We could still be waiting on worker threads to finish - we need another event to tell when REALLY stopped
    ...
    def stop(self):
        ...
        # If there are still workers remaining after the timeout then we remove references to them. We need to do this
        # because workers are daemons but concurrent.futures.thread adds a hook to join all workers with no timeout
        # when shutting down. So any hung worker would prevent the application from shutting down.
        if futures_status.not_done:
            self.logger.warning('There were still unfinished worker threads even after waiting, about to orphan them')

            import concurrent.futures.thread
            self.pool._threads.clear()
            concurrent.futures.thread._threads_queues.clear()

        # Let run() know that we've made our attempt to wait for all the workers and now it's time to die
        self.finished.set()
...
class WrappedTProcessor(TProcessor):
    """Processor that fails gracefully in the event that a handler method raises an unexpected exception"""
    ...
    def handle_exception(self, e, result):
        try:
            ...
            setattr(result, self.default_exception_name,
                    self.default_exception_cls(str(ex) or "No message was found on the raised exception."))
...
def make_server(service, handler, host="127.0.0.1", port=9090, client_timeout=None, cert_file=None):
    ...
    if cert_file:
        server_socket = TSSLServerSocket(host=host, port=port, client_timeout=client_timeout, certfile=cert_file)
    else:
        server_socket = TServerSocket(host=host, port=port, client_timeout=client_timeout)

    return BartenderThriftServer(WrappedTProcessor('baseEx', bg_utils.bg_thrift.BaseException, service, handler),
                                 server_socket)
