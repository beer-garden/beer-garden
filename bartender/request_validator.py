...
class RequestValidator(object):
    ...
    def __init__(self, config):
        ...
        self._client = SystemClient(config.web_host, config.web_port, None, ssl_enabled=config.ssl_enabled,
    ...
    def validate_request(self, request):
        """Validation to be called before you save a request from a user (i.e. through a rest interface)
    ...
    def get_and_validate_system(self, request):
        ...
        if system is None:
            raise BrewmasterModelValidationError("Could not find System named '%s' matching version '%s'" %
                                                 (request.system, request.system_version))

        if request.instance_name not in system.instance_names:
            raise BrewmasterModelValidationError("Could not find instance with name '%s' in system '%s'" %
    ...
    def get_and_validate_command_for_system(self, request, system=None):
        """Ensure the System that will handle this Request has a command with a name that matches this request.
        ...
        for command in system.commands:
            if command.name == request.command:
                ...
                if request.command_type is None:
                    ...
                    self.logger.error("Command Type for Request was %s but the command specified the command_type as "
                                      "%s" % (request.command_type, command.command_type))
                    raise BrewmasterModelValidationError("Command Type for Request was %s but the command specified "
                                                         "the type as %s" %
                                                         (request.command_type, command.command_type))
                ...
                if request.output_type is None:
                    ...
                    self.logger.error("Output Type for Request was %s but the command specified the output_type as "
                                      "%s" % (request.output_type, command.output_type))
                    raise BrewmasterModelValidationError("Output Type for Request was %s but the command specified "
                                                         "the type as %s" %
                                                         (request.output_type, command.output_type))
        ...
        self.logger.error("No Command with name: %s could be found. Valid Commands for %s are: %s" % (
            request.command, system.name, command_names))
        raise BrewmasterModelValidationError("No Command with name: %s could be found. Valid Commands for %s are: %s" %
                                             (request.command, system.name, command_names))

    def get_and_validate_parameters(self, request, command=None, command_parameters=None, request_parameters=None):
        """Validates all request parameters

        Note: It is significant that we build the parameters as a separate object and then assign it.
        This is significant because when you are setting a value to None, mongoengine interprets this as "mark the
        item for deletion" meaning that when you save that attribute, it will actually get deleted. Sounds weird
        right? Well at least you didn't have to troubleshoot it.

        :param request: The Request to validate
        :param command: The the Command for this request. Will attempt to discover if None.
        :param command_parameters:  The command parameters. Will use the Command's parameters if None.
        :param request_parameters: The request parameters. Will use the Request parameters if None.
        ...
        for command_parameter in command_parameters:
            self._validate_required_parameter_is_included_in_request(request, command_parameter, request_parameters)
            extracted_value = self._extract_parameter_value_from_request(request, command_parameter, request_parameters,
                                                                         command)
    ...
    def _validate_value_in_choices(self, request, value, command_parameter):
        ...
        if value is not None and not command_parameter.optional and command_parameter.choices and \
            ...
            if choices.type == 'static':
                if isinstance(choices.value, list):
                    ...
                    if key is None:
                        raise BrewmasterModelValidationError("Unable to validate choices for parameter '%s' - Choices"
                    ...
                    raise BrewmasterModelValidationError("Unable to validate choices for parameter '%s' - Choices value"
                ...
                response_json = json.loads(self._session.get(parsed_value['address'], params=query_params).text)
                ...
                if isinstance(choices.value, six.string_types):
                    ...
                    response = self._client.send_bg_request(_command=parsed_value['name'],
                    ...
                    response = self._client.send_bg_request(_command=parsed_value['name'],
                                                            ...
                                                            _instance_name=choices.value.get('instance_name',
                                                                                             'default'),
                                                            **map_param_values(parsed_value['args']))
                else:
                    raise BrewmasterModelValidationError("Unable to validate choices for parameter '%s' - Choices value"
                ...
                if isinstance(parsed_output, list):
                    if len(parsed_output) < 1:
                        raise BrewmasterModelValidationError("Unable to validate choices for parameter '%s' - Result "
                    ...
                    raise BrewmasterModelValidationError("Unable to validate choices for parameter '%s' - Result of "
                                                         " choices query must be a list" % command_parameter.key)
            else:
                raise BrewmasterModelValidationError("Unable to validate choices for parameter '%s' - No valid type "
            ...
            if command_parameter.multi:
                for single_value in value:
                    if single_value not in allowed_values:
                        msg = "Value '%s' is not a valid choice for parameter with key '%s'. Valid choices are: %s" % \
            ...
            if command_parameter.maximum:
                if isinstance(value, collections.Sequence):
                    if len(value) > command_parameter.maximum:
                        error_msg = "Length %s is greater than the maximum allowed length (%s) for parameter %s" %\
                    ...
                    if value > command_parameter.maximum:
                        error_msg = "Value %s is greater than the maximum allowed value (%s) for parameter %s" %\
            ...
            if command_parameter.minimum:
                if isinstance(value, collections.Sequence):
                    if len(value) < command_parameter.minimum:
                        error_msg = "Length %s is less than the minimum allowed length (%s) for parameter %s" %\
                                          (len(value), command_parameter.minimum, command_parameter.key)
                        self.logger.error(error_msg)
                        raise BrewmasterModelValidationError(error_msg)
                else:
                    if value < command_parameter.minimum:
                        error_msg = "Value %s is less than the minimum allowed value (%s) for parameter %s" %\
        ...
        if value is not None and not command_parameter.optional:
            if command_parameter.regex:
                if not re.match(command_parameter.regex, value):
                    error_msg = "Value %s does not match regular expression %s" % (value, command_parameter.regex)
                    self.logger.error(error_msg)
                    raise BrewmasterModelValidationError(error_msg)

    def _extract_parameter_value_from_request(self, request, command_parameter, request_parameters, command):
        ...
        if command_parameter.multi:
            ...
            if not isinstance(request_values, list):
                raise BrewmasterModelValidationError(
                    "%s was specified as a list, but was not provided as such" % command_parameter.key)

            value_to_return = []
            for value in request_values:
                value_to_return.append(self._validate_parameter_based_on_type(value, command_parameter, command,
                                                                              request))
        else:
            value_to_return = self._validate_parameter_based_on_type(request_value, command_parameter, command, request)

        return value_to_return

    def _validate_required_parameter_is_included_in_request(self, request, command_parameter, request_parameters):
        """If the parameter is required but was not provided in the request_parameters and does not have a default,
        ...
        if not command_parameter.optional:
            if command_parameter.key not in request_parameters and command_parameter.default is None:
                self.logger.error("Required key: %s not provided in request. Parameters are: %s" % (
                    command_parameter.key, request.parameters))
                raise BrewmasterModelValidationError("Required key: %s not provided in request. Parameters are: %s" % (
                    command_parameter.key, request.parameters))

    def _validate_no_extra_request_parameter_keys(self, request_parameters, command_parameters):
        """Validate that all the parameters passed in were valid keys. If there is a key specified that is not noted
        in the database, then a validation error is thrown"""
        ...
        for key in request_parameters:
            if key not in valid_keys:
                self.logger.error("Unknown Key: %s provided in the parameters. Valid Keys are: %s" % (key, valid_keys))
                raise BrewmasterModelValidationError(
                    "Unknown Key: %s provided in the parameters. Valid Keys are: %s" % (key, valid_keys))
    ...
    def _validate_parameter_based_on_type(self, value, parameter, command, request):
        ...
        try:
            if value is None and not parameter.nullable:
                raise BrewmasterModelValidationError(
                    "There is no value for parameter '%s' and this field is not nullable." % parameter.key)
                ...
                if parameter.parameters:
                    ...
                    return self.get_and_validate_parameters(request, command, parameter.parameters, dict_value)
                return dict_value
            ...
            raise BrewmasterModelValidationError(
                "Value for key: %s is not the correct type. Should be: %s" % (parameter.key, parameter.type))
