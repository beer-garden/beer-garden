...
export default function appRun(
  ...
  $rootScope.hasSystemPermission = function(permission, system, global = false) {
    if (!$rootScope.config.authEnabled) return true;
    var garden_name = PermissionService.findGardenScope(null, system.namespace, system.name, system.version);
  ...
  $rootScope.hasCommandPermission = function(permission, command, global = false) {
    ...
    if (!$rootScope.config.authEnabled) return true;
    var garden_name = PermissionService.findGardenScope(null, command.namespace, command.system, command.version);
    return $rootScope.hasPermission(permission, global, garden_name, command.namespace, command.system, command.version, command.name);
  };

  $rootScope.hasRequestPermission = function(permission, request, global = false) {
    if (!$rootScope.config.authEnabled) return true;
    var garden_name = PermissionService.findGardenScope(null, request.namespace, request.system, request.system_version);

    return $rootScope.hasPermission(permission, global, garden_name, request.namespace, request.system, request.system_version, request.command, request.instance_name);
  };
