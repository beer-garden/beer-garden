...
export default function appRun(
  ...
  $rootScope.hasCommandPermission = function(permission, command, global = false) {
    ...
    if (!$rootScope.config.authEnabled) return true;
    var garden_name = PermissionService.findGardenScope(null, command.namespace, command.system, command.instance_name, command.version);
  ...
  $rootScope.hasRequestPermission = function(permission, request, global = false) {
    if (!$rootScope.config.authEnabled) return true;
    var garden_name = PermissionService.findGardenScope(null, request.namespace, request.system, request.instance, request.system_version);
