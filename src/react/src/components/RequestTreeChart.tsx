...
function RequestTreeChart(props: RequestTreeChartProps) {
    ...
    const actionTemplate = (node: any) => {
            ...
            return (
                <div>
                     <Button rounded raised link onClick={() => window.open('/request/' + node.data.id, '_self')} title='Open'><FontAwesomeIcon icon="arrow-up-right-from-square" /> </Button>
                    {!["CANCELED","SUCCESS","ERROR","INVALID"].includes(node.data.status) && <Button rounded raised link onClick={() => {}} title='Cancel'><FontAwesomeIcon icon="ban" /> </Button>}
                    {["CANCELED","SUCCESS","ERROR","INVALID"].includes(node.data.status) && <Button rounded raised link onClick={() => {}} title='Delete'><FontAwesomeIcon icon="trash" /> </Button>}
                    {["CANCELED","SUCCESS","ERROR","INVALID"].includes(node.data.status) && <Button rounded raised link onClick={() => {}} title='Pour Again'><FontAwesomeIcon icon="rotate" /> </Button>}
                    </div>);
        };
    return ( node && 
