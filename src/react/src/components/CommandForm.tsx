...
function CommandForm(commandFormProps: CommandFormProps) {
    ...
    const disabled = commandFormProps.disabled === undefined ? true : commandFormProps.disabled;
    ...
    for (const param of command.parameters || []) {
        ...
        if (request && request.parameters && param.key && (param.key in request.parameters)) {
        ...
        newParam.isInvalid = newParam.value !== undefined || param.optional === undefined || param.optional;

        if ((newParam.value === undefined || newParam.value === null) && param.multi) {
            newParam.value = [null];
        ...
        if (param.type === 'Dictionary'){
            if (param.multi) {
                newParam.value = (newParam.value as Array<any>).map((value) => {return JSON.stringify(value);});
            } else {
        ...
        if (param.type === 'DateTime'){
            if (param.multi) {
                newParam.value = (newParam.value as Array<any>).map((value) => {return new Date(value);});
            } else {
    ...
    const handleChange = (name: any, value: any) => {
        setParameterFields(prevParams => prevParams.map(param => param.key === name ? { ...param, value: value } : param));
    };
                                ...
                                {parameter.value?.map((item: any, index: any) => (
                                    <div key={`${parameter.key}-${index}`} className="dynamic-item">
                                        <InputText
                                            ...
                                            value={item ?? ''}
                                            invalid={!disabled && parameter.optional || undefined}
                                            onChange={(e) => handleMultiChange(parameter.key, index, e.target.value)}
                                            disabled={disabled}
                                        />
                                        <Button 
                                            label="Remove" 
                                ...
                                {parameter.value?.map((item: any, index: any) => (
                                    <div key={`${parameter.key}-${index}`} className="dynamic-item">
                                        <InputNumber
                                            ...
                                            onChange={(e) => handleMultiChange(parameter.key, index, e.value)}
                                            disabled={disabled}
                                        />
                                        <Button 
                                            label="Remove" 
                                ...
                                {parameter.value?.map((item: any, index: any) => (
                                    <div key={`${parameter.key}-${index}`} className="dynamic-item">
                                        <Checkbox
                                            ...
                                            invalid={!disabled && parameter.optional || undefined}
                                            checked={item}
                                            onChange={(e) => handleMultiChange(parameter.key, index, e.checked)}
                                            disabled={disabled}
                                        />
    ...
    return (
        ...
        <DataTable value={parametersFields} showHeaders={false} tableStyle={{ minWidth: '60rem' }}>
