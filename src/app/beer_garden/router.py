...
def determine_route_garden(target_garden_name):
    ...
    if target_garden.has_upstream and target_garden.upstream != config.get("garden.name"):
...
def _pre_forward(operation: Operation) -> Operation:
    ...
    if operation.operation_type == "REQUEST_CREATE":
        ...
        # Clear upstream before forwarding so the downstream garden doesn't freak out about an
        # unknown request
        operation.model.upstream = None
        operation.model.has_upstream = False
