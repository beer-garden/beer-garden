...
def process_publish_event(garden: Garden, event: Event, subscribers: List[Subscriber]):
    ...
    garden_subscribers = [
        ...
        if subscriber.garden is None or len(subscriber.garden) == 0 or garden.name in re.findall(subscriber.garden, garden.name)
    ]
    if garden_subscribers:
        for system in garden.systems:
            system_subscribers = [
                ...
                if (subscriber.system is None or len(subscriber.system) == 0 or system.name in re.findall(subscriber.system, system.name))
                and (subscriber.version is None or len(subscriber.version) == 0 or system.version in re.findall(subscriber.version, system.version))
            ]
            if system_subscribers:
                for command in system.commands:
                    command_subscribers = [
                        ...
                        if subscriber.command is None or len(subscriber.command) == 0 or command.name in re.findall(subscriber.command, command.name)
                    ]
                        ...
                        for instance in system.instances:
                            if instance.status == "RUNNING":
                                instance_subscribers = [
                                    subscriber
                                    for subscriber in system_subscribers
                                    if subscriber.instance is None or len(subscriber.instance) == 0 or instance.name in re.findall(subscriber.instance, instance.name)
                                ]
