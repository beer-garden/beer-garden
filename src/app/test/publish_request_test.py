...
class TestSubscriptionEvent(object):
    ...
    def test_topic_two(self, monkeypatch, localgarden):
        ...
        event = Event(name=Events.REQUEST_TOPIC_PUBLISH.name, metadata={"propagate": False, "topic":"topic_2"}, payload=Request())
        beer_garden.publish_request.process_publish_event(localgarden, event)
    ...
    def test_topic_wildcard(self, monkeypatch, localgarden):
        ...
        event = Event(name=Events.REQUEST_TOPIC_PUBLISH.name, metadata={"propagate": False, "topic":"topic_3"}, payload=Request())
        beer_garden.publish_request.process_publish_event(localgarden, event)
    ...
    def test_topic_non_match(self, monkeypatch, localgarden):
        ...
        event = Event(name=Events.REQUEST_TOPIC_PUBLISH.name, metadata={"propagate": False, "topic":"badTopic"}, payload=Request())
        beer_garden.publish_request.process_publish_event(localgarden, event)
