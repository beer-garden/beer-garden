...
class RequestListAPITest(unittest.TestCase):
    ...
    def test_get_emtpy(self, objects_mock):
        fake_list = Mock(
            __iter__=Mock(return_value=[]), __getitem__=Mock(return_value=[])
        )
    ...
    def test_get_with_results_no_headers(self, all_mock):
        fake_list = Mock(
            __iter__=Mock(return_value=[self.default_request]),
    ...
    def test_get_response_headers_datatables(self):
        ...
        query_params = {"draw": "1", "start": "0", "length": "10"}

        query_mock = self._get_query_mock()
        with patch("bg_utils.mongo.models.Request.objects", return_value=query_mock):
            rv = self.app.get(
                "/api/v1/requests?" + self._datatables_query(query_params=query_params)
            )
    ...
    def test_get_with_columns(self):
        ...
        query_mock = self._get_query_mock()
        with patch("bg_utils.mongo.models.Request.objects", return_value=query_mock):
            self.app.get("/api/v1/requests?" + self._datatables_query(columns=columns))
    ...
    def test_no_search_params(self):
        ...
        query_mock = self._get_query_mock()
        with patch(
            "bg_utils.mongo.models.Request.objects", return_value=query_mock
        ) as search_mock:
            self.app.get("/api/v1/requests")

            self.assertEqual(
                search_mock.call_args[0][0].query, Q(parent__exists=False).query
            )
    ...
    def test_get_overall_search(self):
        ...
        query_params = {"search": json.dumps({"value": search_value, "regex": "false"})}
    ...
    def test_get_order_by_asc(self):
        ...
        query_params = {"order": json.dumps({"column": 0, "dir": "asc"})}
    ...
    def test_get_order_by_desc(self):
        ...
        query_params = {"order": json.dumps({"column": 0, "dir": "desc"})}
    ...
    @patch(
        "bg_utils.parser.BeerGardenParser.parse_request_dict",
    ...
    def test_post_invalid_message(self, parse_mock):
        brew_view.backend.validateRequest.return_value = Mock(
    ...
    @patch("bg_utils.parser.BeerGardenParser.parse_request_dict")
    @patch(
        "brew_view.controllers.request_list_api.url_for", Mock(return_value="some_url")
    )
